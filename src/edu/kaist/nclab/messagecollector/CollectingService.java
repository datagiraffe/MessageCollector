...
public class CollectingService extends AccessibilityService {
		...
		if (isKakaoTalkMessage(event)) {
			...
			LocationResult locationResult = new LocationResult() {
				...
				public void gotLocation(Location location) {
					...
					Log.v(TAG, "Message: " + messageContent + " at: "
							+ messageTime + " while seeing " + foregroundApp
	...
	/**
	 * Checks whether the given event is a KakaoTalk Popup message of message
	 * length greater than 2 (only those with content)
	 * 
	 * @param event
	 *            the captured AccessibilityEvent
	 */
	private boolean isKakaoTalkMessage(AccessibilityEvent event) {
		...
		return (event.getEventType() == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED
				&& event.getText().toString().length() > 2 && eventPackageName
					.contentEquals("com.kakao.talk"));
	}
	...
	private String getForegroundApplicationName() {
		ActivityManager actMng = (ActivityManager) this
